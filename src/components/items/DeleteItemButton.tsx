...
const variantClasses: Record<NonNullable<DeleteItemButtonProps['variant']>, string> = {
  default:
  ...
  return (
    <ConfirmDialog
      ...
      trigger={
        <button
          type="button"
          className={`${variantClasses[variant]} ${disabled ? 'cursor-not-allowed opacity-60' : ''}`.trim()}
          disabled={disabled}
        >
          {t('trigger')}
        </button>
